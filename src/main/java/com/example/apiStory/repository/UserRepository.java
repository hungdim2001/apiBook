...
import com.example.apiStory.dto.UserDto;
...
public interface UserRepository extends JpaRepository<User, Long>, JpaSpecificationExecutor<User> {
    ...
    @Query(value = "SELECT new com.example.apiStory.dto.UserDto(u, ur.role )  FROM User u, UserRole ur WHERE  u.username =?1 OR u.email = ?1 and u.id = ur.userId")
    Optional<UserDto> findUserAndRole(String account);
    @Query(value = "SELECT new com.example.apiStory.dto.UserDto(u, ur.role )  FROM User u, UserRole ur WHERE  u.id = ?1 and u.id = ur.userId")
